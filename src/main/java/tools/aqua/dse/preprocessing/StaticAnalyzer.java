...
//        }
     ...
     * x!3 = declaring klass
     */
    public String generatePolymorphismSummary(ClassType[] types) {
        StringBuilder result = new StringBuilder();
    ...
    public List<Trace> performDseOnConstructor(String constructorSignature) {
...
//                "-Dconcolic.constructors=%s " +
    ...
    public String generateSmtCodeBluePrintForConstructorSelection(int depth) {
        List<String> signaturesConstructorCalls = this.generateSignaturesOfPossibleConstructorCallsFromNondetObject(depth);
        ...
        this.declarations.addAll(traces.stream()
    ...
    public String generateSmtCodeFromConstructorSummaryTraces(List<Trace> summaryTraces) {
        return String.format(
                "(assert (or %n%s%n))",
                summaryTraces.stream()
                        .map(this::generateSmtCodeFromSingleConstructorSummaryTrace)
                        .collect(Collectors.joining("\n"))
        );
    }

    public String generateSmtCodeFromSingleConstructorSummaryTrace(Trace summaryTrace) {
        return String.format(
                "\t(and %s)",
                Stream.concat(
    ...
    public List<String> possibleObjectsFromNondetObjectWithDepth(int depth) {
